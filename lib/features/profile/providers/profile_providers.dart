...
/// Active local user — auto-created on first launch (no onboarding wizard).
final activeUserProvider = FutureProvider<User?>((ref) async {
  final result = await ref.watch(ensureLocalUserProvider)(const NoParams());
  return result.valueOrNull;
});

final hasCompletedOnboardingProvider = Provider<AsyncValue<bool>>((ref) {
  return ref.watch(activeUserProvider).whenData((user) => user != null);
});
